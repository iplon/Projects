...
public function getalarms()
	 {
		...
		$type="";
		$msg=array();
		$slq = "Select a.ts,a.blockname,a.device_name,a.field,a.value,b.history_enable,b.alarm_type,b.type,b.priority from scada_latest_igate_data as a join alarm_fields as b on(a.field=b.alarm_field) where b.status='1' and (b.type='process' or b.type='fire') and a.blockname!='' and a.device_name!='' and a.ts!='0' and value NOT IN ('nan','NaN')";
						...
						elseif (in_array($val, $chkarr))
						{
							...
							if($numq>0)
							{
								...
								while($fetchq=mysql_fetch_array($sl))
								{
									...
									if($stas==2)
									{
										echo $insqry="Update ".$this->table." set solved_datetime=now(),status=1 where id='".$ids."'";	
									}
									else
									{
		...
		//audio alarm hooter control start///
		$dtm=date("Y-m-d");
		$cSession = curl_init();
		...
		$audAlm="select b.id as count FROM alarm_fields as a join alarm as b on(a.alarm_field=b.field) where b.date='".$dtm."' and a.type='process' and a.status='1' and a.audio_status='1' and b.reset='1'";
		...
		if($numq>0){	
			...
			$url="http://localhost:3000/scaback0001/setProp?id=".$id."&val=2";
				...
				curl_close($cSession);
			//audio alarm hooter control end///
