<?php
   ...
   else
   {
	...
	private function station()
	{
		...
		$sclqry1="select blockname,device_name,field,FROM_UNIXTIME($startTime) AS ts, value FROM scada_latest_igate_data where ts!='' $where1 AND field IN('EAE','EAI','EAN') order by field";
			$sql1= mysql_query($sclqry1, $this->db);
			if(mysql_num_rows($sql1) > 0)
			{
				while($rlt1 = mysql_fetch_array($sql1,MYSQL_ASSOC)){
	...
	/*STATION REPORT END*/
		...
		if($tpe=="month")
		{
			...
			$sclqry="SELECT block,device,field,DATE(FROM_UNIXTIME(ts)) AS ts,value FROM day_variable where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 order by ts,block,device";
			...
			if(mysql_num_rows($sql) > 0)
			{
  			...
  			$sclqry1="SELECT block,device,'MAX',DATE(FROM_UNIXTIME(ts)) AS ts,MAX(ABS(value)) AS value FROM day_variable where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 GROUP BY device,block ORDER BY ts,block,device";
			$sql1 = mysql_query($sclqry1, $this->db);
			if(mysql_num_rows($sql1) > 0)
			{
				while($rlt1 = mysql_fetch_array($sql1,MYSQL_ASSOC))
			...
			$sclqry3="SELECT block,device,'MIN',DATE(FROM_UNIXTIME(ts)) AS ts,MIN(ABS(value)) AS value FROM day_variable where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 GROUP BY device,block ORDER BY ts,block,device";
			...
			$sclqry2="SELECT block,device,'TOTAL',DATE(FROM_UNIXTIME(ts)) AS ts,SUM(ABS(value)) AS value FROM day_variable where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' and value!='nan' $where1 GROUP BY device,block ORDER BY ts,block,device";
			$sql2 = mysql_query($sclqry2, $this->db);
			if(mysql_num_rows($sql2) > 0)
			{
				while($rlt2 = mysql_fetch_array($sql2,MYSQL_ASSOC))
		...
		elseif($tpe=="year")
		{
			...
			$sclqry="SELECT block,device,field,FROM_UNIXTIME(ts,'%Y-%m') AS ts,SUM(ABS(value)) AS value FROM day_variable where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 group by MONTH(FROM_UNIXTIME(ts)),device,block order by ts,block,device";
			...
			if(mysql_num_rows($sql) > 0)
			{
 	 		...
 	 		$sclqry1="SELECT block,device,'MAX',FROM_UNIXTIME(ts,'%Y') AS ts,MAX(ABS(value)) AS value FROM day_variable where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 GROUP BY MONTH(FROM_UNIXTIME(ts)),device,block ORDER BY ts,block,device";
			$sql1 = mysql_query($sclqry1, $this->db);
			if(mysql_num_rows($sql1) > 0)
			{
				while($rlt1 = mysql_fetch_array($sql1,MYSQL_ASSOC))
			...
			$sclqry3="SELECT block,device,'MIN',FROM_UNIXTIME(ts,'%Y') AS ts,MIN(ABS(value)) AS value FROM 
			where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 GROUP BY MONTH(FROM_UNIXTIME(ts)),device,block ORDER BY ts,block,device";
			$sql3 = mysql_query($sclqry3, $this->db);
			if(mysql_num_rows($sql3) > 0)
			{
				while($rlt3 = mysql_fetch_array($sql3,MYSQL_ASSOC))
			...
			$sclqry2="SELECT block,device,'TOTAL',FROM_UNIXTIME(ts,'%Y') AS ts,SUM(ABS(value)) AS value FROM day_variable where ts >= $startTime and ts < $endTime AND block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' and value!='nan' $where1 GROUP BY device,block ORDER BY ts,block,device";
			$sql2 = mysql_query($sclqry2, $this->db);
			if(mysql_num_rows($sql2) > 0)
			{
				while($rlt2 = mysql_fetch_array($sql2,MYSQL_ASSOC))
		...
		elseif($tpe=="tilldate")
		{
			$sclqry="SELECT block,device,field,FROM_UNIXTIME(ts,'%Y') AS ts,SUM(ABS(value)) AS value FROM day_variable where block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' and value!='nan' $where1 group by YEAR(FROM_UNIXTIME(ts)),device,block order by ts,block,device";
			$sql = mysql_query($sclqry, $this->db);
			if(mysql_num_rows($sql) > 0)
			{
				while($rlt = mysql_fetch_array($sql,MYSQL_ASSOC))
 	 		...
 	 		$sclqry1="SELECT block,device,'MAX',FROM_UNIXTIME(ts,'%Y-%m') AS ts,MAX(ABS(value)) AS value FROM day_variable where block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1  ORDER BY ts,block,device";
			$sql1 = mysql_query($sclqry1, $this->db);
			if(mysql_num_rows($sql1) > 0)
			{
				while($rlt1 = mysql_fetch_array($sql1,MYSQL_ASSOC))
			...
			$sclqry3="SELECT block,device,'MIN',FROM_UNIXTIME(ts,'%Y-%m') AS ts,MIN(ABS(value)) AS value FROM day_variable where block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 ORDER BY ts,block,device";
			$sql3 = mysql_query($sclqry3, $this->db);
			if(mysql_num_rows($sql3) > 0)
			{
				while($rlt3 = mysql_fetch_array($sql3,MYSQL_ASSOC))
			...
			$sclqry2="SELECT block,device,'TOTAL',FROM_UNIXTIME(ts,'%Y-%m') AS ts,SUM(ABS(value)) AS value FROM day_variable where block NOT LIKE 'ALL_BLOCK' and field='EAE_DAY' AND device LIKE '%ALL_INV%' and value!='nan' $where1 GROUP BY device,block ORDER BY ts,block,device";
			$sql2 = mysql_query($sclqry2, $this->db);
			if(mysql_num_rows($sql2) > 0)
			{
				while($rlt2 = mysql_fetch_array($sql2,MYSQL_ASSOC))
		...
		{	
			...
			$sclqry1="SELECT block,device,field,FROM_UNIXTIME(ts,'%Y-%m-%d %H:%i') AS ts,value FROM day_variable where ts = $startTime and field='PAC_MAX' AND device LIKE '%ALL_INV%' $where1 order by block,device";
			$sql1 = mysql_query($sclqry1, $this->db);
			if(mysql_num_rows($sql1) > 0)
			{
				while($rlt1 = mysql_fetch_array($sql1,MYSQL_ASSOC))
			...
			$sclqry2="SELECT block,device,field,FROM_UNIXTIME(ts,'%Y-%m-%d %H:%i') AS ts,value FROM day_variable where ts = $startTime AND block NOT LIKE 'ALL_BLOCK' AND field='EAE_DAY' AND device LIKE '%ALL_INV%' $where1 order by block,device";
			$sql2 = mysql_query($sclqry2, $this->db);
			if(mysql_num_rows($sql2) > 0)
			{
				while($rlt2 = mysql_fetch_array($sql2,MYSQL_ASSOC))
  ...
  private function add_event()
		...
		private function report_kv()
	{
		...
		$sclqry3="SELECT block,device,field,FROM_UNIXTIME(ts,'%Y-%m-%d %06:%00') AS ts,value FROM day_variable where ts = $startTime and field IN('PAC_MAX','SOLAR_RADIATION_MAX') AND device IN('".$incommers."','CR_WS') order by FIELD(device, '".$incommers."','CR_WS')";
		$sql3 = mysql_query($sclqry3, $this->db);
		if(mysql_num_rows($sql3) > 0)
		{
			while($rlt3 = mysql_fetch_array($sql3,MYSQL_ASSOC)){
		...
		private function report_inverter_hour()
	{
		...
		if(isset($_REQUEST['block']))
		{
			...
			if(($blk!='')&&($blk!='All'))
			{
				...
				$where.=" and (block IN ('".$blkchk."','WS','CR'))";
			}
		}		
		$result = array();
		$result1 = array();
...
$sclqry1="select 'All_Block','All_Inverter','Total',FROM_UNIXTIME(hourts,'%Y-%m-%d %H:%i') AS hourts,Sum(value) as value FROM hourly_compressedvalue where ts='".$startTime."' and block!='WS' and display like '%Inverter%' and field='EAE' group by field,hour order by `block`,`display`,`hour`";
	...
	private function mainpage()
	{
		...
		$sclqry2="select a.blockname,a.device_name,a.field,a.value FROM scada_latest_igate_data as a where a.ts!='' and blockname='CR' and device_name='CR_WS' and field='SOLAR_RADIATION_CUM'";
		$sql2 = mysql_query($sclqry2, $this->db);
		if(mysql_num_rows($sql2) > 0)
		{
			while($rlt2 = mysql_fetch_array($sql2,MYSQL_ASSOC)){
...
private function systemarchitCR()
 {
  ...
  $selectQuery3 ="select blockname,device_name,value from scada_latest_igate_data where blockname IN ('CR','WS') and device_name IN('IO','CRP_IO','CR_WS') and field ='COMMUNICATION_STATUS' order by blockname";
  ...
  if(mysql_num_rows($sql3) > 0)
  {
	...
	private function mis_graph()
	{
		...
		if(isset($_REQUEST['device']))
		{
			...
			{
				$divchk=str_replace(",","','",$dvfld);
				$where=" and (device_name IN ('".$divchk."','CR_WS'))";
   ...
   else
   {
	   $startTime = mktime(0, 0, 0, $slmnth, $sdd, $crtyr);
		...
		$sclqry="select insertedts,device_name,field,value as value from ".$tbl." where ts >= $startTime and ts < $endTime AND field IN ('EAE_DAY','SOLAR_RADIATION','PAC') $groupby order by FIELD(field, 'PAC','SOLAR_RADIATION','EAE_DAY'),ts";
		}
   		$result = array();
		$sql = mysql_query($sclqry, $this->db);
		if(mysql_num_rows($sql) > 0)
	...
	private function report_mainpage()
		...
		if(isset($_REQUEST['device']))
		{
			...
			if(($dvfld!='')&&($dvfld!='All'))
			{
				$divchk=str_replace(",","','",$dvfld);
				$where.=" and (device IN ('".$divchk."'))";
				$where1.=" and (device IN ('".$divchk."','CR_WS','CR_IO'))";
			}
		}
		if((isset($_REQUEST['type']))&&($_REQUEST['type']!=''))
		{
		...
		$sclpac="select block,device,field,FROM_UNIXTIME(ts) as ts,value FROM `day_variable` WHERE ts='".$startTime."' $where1 and `field`IN('PAC_MAX','PR_DAY','SOLAR_RADIATION_CUM','SOLAR_RADIATION_MAX','SOLAR_RADIATION_AVG','AIR_TEMP_AVG','OPERATIONAL_TIME') order by FIELD(field, 'PAC_MAX','PR_DAY','SOLAR_RADIATION_CUM','SOLAR_RADIATION_MAX','SOLAR_RADIATION_AVG','AIR_TEMP_AVG','OPERATIONAL_TIME')";
		...
		{ 
			...
			{ 
				...
				if(($fld=='')||($fld=='All')) 
				{ 
					$where.=" and (field IN ('EAI','PAC','EAE','SOLAR_RADIATION','Solar_Radiation_5min_AVG','AIR_TEMP','Wind_Speed','AIR_PRESSURE'))"; 
				} 
				else
				{
					$arrfld=str_replace(",","','",$fld); 
			...
			{
				$result = array();
			$sclqry="select blockname,device_name,field,FROM_UNIXTIME(insertedts),round(value,3) as value from ".$tbl." where ts >= $startTime and ts < $endTime and (device_name IN ('$this->lf','CR_WS')) $where GROUP BY ( 4 * HOUR(from_unixtime(insertedts)) + FLOOR( MINUTE(from_unixtime(insertedts)) / 15 )),field,device_name,blockname order by FIELD(field, 'EAI','PAC','EAE','SOLAR_RADIATION','Solar_Radiation_5min_AVG','AIR_TEMP','Wind_Speed','AIR_PRESSURE'),insertedts,device_name,blockname";
				$sql = mysql_query($sclqry, $this->db) or die(mysql_error()); 
				if(mysql_num_rows($sql) > 0)
				{ 
					while($rlt = mysql_fetch_array($sql,MYSQL_ASSOC))
				...
				$sclqry2="select blockname,device_name,field,FROM_UNIXTIME(insertedts),round(value,3) from ".$tbl." where ts >= $startTime and ts < $endTime and device_name = 'CR_WS' and field = 'MODULE_TEMP_AVG', GROUP BY insertedts,field order by insertedts";
				$sql2 = mysql_query($sclqry2, $this->db); 
				if(mysql_num_rows($sql2) > 0)
				{ 
					while($rlt2 = mysql_fetch_array($sql2,MYSQL_ASSOC))
