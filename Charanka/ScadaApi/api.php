<?php
		 ...
		 */
		 ...
		 private function graph() {
			 ...
			 if(isset($_REQUEST['field'])) {
				 ...
				 if(($fld!='')&&($fld!='All')) {
					 ...
					 if((($blk!='')&&($blk!='All'))||(($dvfld!='')&&($dvfld!='All'))) {
						 $where.=" and field IN ('".$arrfld."')";
					 }
					 else {
						 $where.="field IN ('".$arrfld."')";
					 }
			 ...
			 if($fld!='') {
				 if(($month==date("m")) && ($year==date("Y"))) {
					 ...
					 $groupby="GROUP BY (12 * HOUR(FROM_UNIXTIME(insertedts)) + FLOOR(MINUTE(FROM_UNIXTIME(insertedts))/ 5)), DATE(FROM_UNIXTIME(insertedts)),field,device_name,blockname";
				 }
				 $sclqryt="SELECT GROUP_CONCAT(`blk_dv_fld_id` ORDER BY $block, $device, $field separator ',') FROM fields_num WHERE $where";
				 $sql = mysql_query($sclqryt, $this->db);
				 $sclqryt=mysql_result($sql,0);
				 mysql_free_result($sql);
				 $sclqry="SELECT a.blockname,a.device_name,a.field,b.insertedts,b.value FROM fields_num AS a JOIN $tbl AS b ON (a.blk_dv_fld_id=b.blk_dv_fld_id) WHERE a.blk_dv_fld_id IN ($sclqryt) AND b.insertedts >= $startTime AND b.insertedts <$endTime $groupby ORDER BY FIELD (a.blk_dv_fld_id, $sclqryt),b.insertedts";
		 ...
		 private function flexible() {
			 ...
			 if(isset($_REQUEST['text'])) {
				 $text='_text';
				 $lookuptbl="fields_text";
			 }
			 else {
				 $text='';
				 $lookuptbl="fields_num";
			 }
			 ...
			 if($fld!='') {
				 ...
				 $sclqryt="SELECT GROUP_CONCAT(`blk_dv_fld_id` ORDER BY $block, $device, $field separator ',') FROM $lookuptbl WHERE $where";
				 $sql = mysql_query($sclqryt, $this->db);
				 $sclqryt=mysql_result($sql,0);
				 mysql_free_result($sql);
				 $sclqry="SELECT a.blockname,a.device_name,a.field,FROM_UNIXTIME(b.insertedts,'%Y-%m-%d %H:%i') AS ts,b.value FROM $lookuptbl AS a JOIN $tbl AS b ON (a.blk_dv_fld_id=b.blk_dv_fld_id) WHERE a.blk_dv_fld_id IN ($sclqryt) AND insertedts >= $startTime AND insertedts <$endTime $groupby ORDER BY FIELD (a.blk_dv_fld_id, $sclqryt),b.insertedts";
				 $sql = mysql_query($sclqry, $this->db);
				 if(mysql_num_rows($sql) > 0) {
					 while($rlt = mysql_fetch_array($sql,MYSQL_ASSOC))
					 $result[]=array_values($rlt);
 ...
 $scl1="SELECT plant,'CR_EM','EAE_DAY',FROM_UNIXTIME(ts, '%Y-%m-%d') AS ts,cross_export FROM abt_revenue where ts >= $startTime and ts < $endTime $device order by ts";
  ...
  private function add_event()
	...
	if($today==$startTime)
	{
	$sclqry="select a.Block,a.Device,CASE a.StartTime WHEN '0' THEN 'Null' ELSE a.StartTime END,CASE a.StopTime WHEN '0' THEN 'Null' ELSE a.StopTime END,a.EnergyGeneration,a.PeakPAC,a.remark,'' FROM inverter_dcframe_report as a where a.ReportDate='".$dt."' $where order by a.Block,LPAD(a.Device,20,0)";
	}
	else
	{
	$sclqry="select a.Block,a.Device,CASE a.StartTime WHEN '0' THEN 'Null' ELSE a.StartTime END,CASE a.StopTime WHEN '0' THEN 'Null' ELSE a.StopTime END,a.EnergyGeneration,a.PeakPAC,a.remark,a.ts FROM inverter_dcframe_report as a where a.ReportDate='".$dt."' $where order by a.Block,LPAD(a.Device,20,0)";
	}
		$sql = mysql_query($sclqry, $this->db);
		if(mysql_num_rows($sql) > 0)
		{
		...
		private function report_kv()
	{
		...
		$sclqry="select a.DisplayName,a.Device,'EAE',FROM_UNIXTIME(a.lastts,'%Y-%m-%d %H:%i') AS ts,a.value FROM accord_em_report as a where ReportDate='".$startTime."'  and field='EAE_DAY' and timeinhour >=6 and timeinhour <=19 $where order by Device,DisplayName,timeinhour";
		$sql = mysql_query($sclqry, $this->db);
		...
		$sclqry3="SELECT block,device,field,FROM_UNIXTIME(ts,'%Y-%m-%d %06:%00') AS ts,value FROM day_variable where ts = $startTime and field IN('PAC_MAX','SOLAR_RADIATION_MAX') AND device IN('".$incommers."','CR_WS') order by FIELD(device, '".$incommers."','WS')";
		$sql3 = mysql_query($sclqry3, $this->db);
		if(mysql_num_rows($sql3) > 0)
		{
			while($rlt3 = mysql_fetch_array($sql3,MYSQL_ASSOC)){
			 $result[]=array_values($rlt3);
			}
		}
...
$sclqry1="select 'All_Block','All_Inverter','Total',FROM_UNIXTIME(hourts,'%Y-%m-%d %H:%i') AS hourts,Sum(value) as value FROM hourly_compressedvalue where ts='".$startTime."' and block!='WS' and display like '%Inverter%' and field='EAE' group by field,hour order by `block`,`display`,`hour`";
	...
	private function mainpage()
	{
		...
		$sclqry2="select a.blockname,a.device_name,a.field,a.value FROM scada_latest_igate_data as a where a.ts!='' and blockname='WS' and device_name='WS' and field='PLANT_AUXILIARY'";
		$sql2 = mysql_query($sclqry2, $this->db);
		if(mysql_num_rows($sql2) > 0)
		{
			while($rlt2 = mysql_fetch_array($sql2,MYSQL_ASSOC)){
		...
		$energy_meters=array_filter(array($this->lf,$this->lf1));
...
if($tpe=="month")
   {
	...
	$startTime = mktime(0, 0, 0, $slmnth, $sdd, $crtyr);
	$endTime = mktime(0, 0, 0, $slmnth+1, $sdd, $crtyr);
	$sclqry="SELECT ts,device,field,value FROM `day_variable`
	WHERE ts >= $startTime and ts < $endTime and field IN('EAE_DAY','RADIATION_CUM','PR_DAY','PLANT_AUXILIARY','PLANT_AVAILABILITY','GRID_AVAILABILITY') $where1 order by FIELD(field,'EAE_DAY','RADIATION_CUM','PR_DAY','PLANT_AUXILIARY','PLANT_AVAILABILITY','GRID_AVAILABILITY'),ts";
   ...
   else if($tpe=="year")
   {
	...
	$startTime = mktime(0, 0, 0, $slmnth, $sdd, $crtyr);
	$endTime = mktime(0, 0, 0, $slmnth, $sdd, $crtyr+1);
	$sclqry="SELECT FROM_UNIXTIME(ts, '01-%m-%Y') as ts,device,field,CASE WHEN field LIKE 'PR_DAY' THEN AVG(ABS(value))
	 ELSE SUM(ABS(value)) END AS value FROM `day_variable`
	 WHERE ts >= $startTime and ts < $endTime and field IN('EAE_DAY','RADIATION_CUM','PR_DAY','PLANT_AUXILIARY','PLANT_AVAILABILITY','GRID_AVAILABILITY') $where1  group by Month(FROM_UNIXTIME(ts)),field order by FIELD(field, 'EAE_DAY','RADIATION_CUM','PR_DAY','PLANT_AUXILIARY','PLANT_AVAILABILITY','GRID_AVAILABILITY'),ts";
   }
    else if($tpe=="week")
   {
		...
		$endTime = mktime(0, 0, 0, $slmnth, $sdd+1, $crtyr);
		$sclqry="SELECT ts,device,field,value FROM `day_variable`
		WHERE ts >= $startTime and ts < $endTime and
		field IN('EAE_DAY','RADIATION_CUM','PR_DAY','PLANT_AUXILIARY','PLANT_AVAILABILITY','GRID_AVAILABILITY') $where1 order by FIELD(field, 'EAE_DAY','RADIATION_CUM','PR_DAY','PLANT_AUXILIARY','PLANT_AVAILABILITY','GRID_AVAILABILITY'),ts";
   }
   else
   {
	   $startTime = mktime(0, 0, 0, $slmnth, $sdd, $crtyr);
		...
		$sclqry="select a.insertedts,b.device_name,b.field,a.value as value from ".$tbl." AS a JOIN fields_num AS b ON (a.blk_dv_fld_id=b.blk_dv_fld_id)  
		where a.insertedts >= $startTime and a.insertedts < $endTime AND b.field IN ('PAC','RADIATION') $where $groupby order by b.device_name";
   }
   		$result = array();
		$sql = mysql_query($sclqry, $this->db);
		if(mysql_num_rows($sql) > 0)
	...
	private function report_mainpage()
		...
		$sclpac="select block,device,field,FROM_UNIXTIME(ts) as ts,value FROM `day_variable` WHERE ts='".$startTime."' $where1 and `field`IN('PLANT_AVAILABILITY','PAC_MAX','PR_DAY','SOLAR_RADIATION_CUM','SOLAR_RADIATION_MAX','SOLAR_RADIATION_AVG','AMBIENT_TEMP_AVG','OPERATIONAL_TIME','PLANT_AUXILIARY') order by FIELD(field, 'PAC_MAX','PR_DAY','RADIATION_CUM','SOLAR_RADIATION_MAX','SOLAR_RADIATION_AVG','AMBIENT_TEMP_AVG','OPERATIONAL_TIME','PLANT_AUXILIARY')";
