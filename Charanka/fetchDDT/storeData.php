	...
	public function storeData()	{
		...
		if($num>0)	{
		    ...
		    $part=(int)($dpt/$dpp) + 2;
				...
				$sqlqry="SELECT blk_dv_fld_id,ts,blockname,device_name,field,value,format,graph FROM latest_igate_data WHERE ts >= $this->startCheck AND blk_dv_fld_id!=0 AND blockname!='' AND device_name!='' AND field!='' ORDER BY format,field,device_name,blockname";
				$qry= mysql_query($sqlqry);
				$num=mysql_num_rows($qry);
				if($num>0)	{
					if ($min%5=='01')	{
