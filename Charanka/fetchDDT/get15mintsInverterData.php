		...
		while($fetchel=mysql_fetch_array($qryel))
			{
				...
				$fldel=$fetchel['field'];
				$valel=$fetchel['value'];
				mysql_query("Replace ".$this->table." (ts,blockname,device_name,field,value,insertedts) values 
				('".$tsel."','".$ablk."','".$eldv."','".$fldel."','".$valel."','".$this->nowtime."')");
			}
	}
}
 ...
 public function get15mintsInverterData()
	{
		...
		if($numarr >0)
		{
			for($n=0;$n<$numarr;$n++)
			{
				...
				{
					$dv=$arrd[$i];
					$sqlqry="select ts,field,SUM(value+0.0)as value from ".$this->tablefrm." 
					where blockname='".$ablk."' and device_name like '%".$dv."' and ((field='PAC') or (field='PDC') or (field='EAE') or (field='IDC') or (field='IGBT_TEMP') or (field='UDC')) group by field";
					// echo $sqlqry."<br>";
					...
					if($num>0)
					{
						while($fetch=mysql_fetch_array($qry))
						{
							...
							$type=$arrdevice[$i];
							// echo $ts=$fetch['ts'];
							// echo $fld=$fetch['field'];
							// echo $val=$fetch['value'];
							// echo $type=$arrdevice[$i];
							// echo "success";
							// exit();					
							...
							$insqry1 = "REPLACE ".$this->table." (ts,blockname,device_name,field,value,insertedts) values ('".$ts."','".$ablk."','".$type."','".$fld."','".$val."','".$this->nowtime."')";
							mysql_query($insqry1);
							echo $insqry1;
