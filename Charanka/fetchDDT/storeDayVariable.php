...
{
		$msg=array();
		$slq = "SELECT a.blockname,a.device_name,a.field,a.value from scada_latest_igate_data AS a join day_variable_fields AS b ON(a.field=b.field)
		WHERE b.status='1' AND a.ts >= $this->startTime AND a.ts < $this->endTime GROUP BY a.field, a.device_name, a.blockname 
		order by a.blockname,a.device_name,a.field";
		echo $slq;
				...
				if($value=='nan' || $value=='NaN' || $value=='0'){
					...
					$num1=mysql_num_rows($sQuery1);
						if($num1!=0){
							...
							$val=stripslashes(trim(str_replace("'","",$fetch1['value'])));
							$msg[]="('".$this->startTime."','".$blk."','".$dev."','".$fld."','".$val."')";
						}
						else{
							$val='0';	
