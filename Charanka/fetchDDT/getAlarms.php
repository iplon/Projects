...
public function getalarms()
	 {
		...
		WHERE a.value NOT IN('0', '', 'NaN') AND b.status = '1' AND (b.type = 'process' OR b.type = 'fire') GROUP BY a.blockname,a.device_name,a.field";
		// echo $sql;
		// exit;
	...
	foreach($alarmStringAry as $index=>$string){
					if(isset($string) && $string!=""){
						foreach($string as $key => $value){
							...
							$slq = "Select a.ts,a.blockname,a.device_name,a.field,b.history_enable,b.alarm_type,b.type,b.priority from scada_latest_igate_data as a join 
		...
		if($num >0)
		{
			while($fetch=mysql_fetch_array($sQuery)) 
			{
					...
					$val=stripslashes(trim(str_replace("'","",$value)));
