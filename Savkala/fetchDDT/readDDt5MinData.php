...
if($num>0)
{
	...
	$data=implode(",",$msg);
	$insq="Replace into 5min_data_unique (ts,blk_dv_fld_id,value,insertedts) values  ".$data;
	echo $insq."<br>";
	$insertsq=mysql_query($insq) or die(mysql_error());
	
	if(($chr=='23')&&($min>='55'))
	{
		$trc="truncate table 5min_data_today_unique";
		$trnt=mysql_query($trc) or die(mysql_error());
		
	}
	else
	{
		$insq2="Replace into 5min_data_today_unique (ts,blk_dv_fld_id,value,insertedts) values  ".$data;
		$insertsq2=mysql_query($insq2) or die(mysql_error());
		if(($chr=='00')&&($min<='15'))
		{
			mysql_query("ANALYZE TABLE 5min_data_today_unique");
			mysql_query("OPTIMIZE TABLE 5min_data_today_unique");
			mysql_query("FLUSH TABLE 5min_data_today_unique");
		}
	}
	...
	{
		$renametbl="5min_data_unique";
		$newname="5min_data_unique_".date("M_Y");
		$renm="RENAME TABLE `" . $renametbl . "` TO `" . $newname . "`";
		$reqry=mysql_query($renm) or die(mysql_error());
		if($reqry)
		{
			$csql = "CREATE TABLE IF NOT EXISTS `5min_data_unique` (
			`ts` int(14) NOT NULL,
