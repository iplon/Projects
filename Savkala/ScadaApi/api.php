  ...
  private function add_event()
		...
		private function acknowledge()
		{
			...
			if($alarmId!='')
			{
				...
				////buzzer controll start////
					...
					$ffields[]=$ftq['alarm_field'];
					} 
					$ffields = implode(',',$ffields);
					$ffields = "'".str_replace(",","','",$ffields)."'";
					}
				
				 $qy1="select id from scada_latest_igate_data where blockname LIKE 'CR%' and device_name LIKE '%IO' and field='FIRE_ALARM_CMD' limit 1";
				$qrysl= mysql_query($qy1);
				$numsl=mysql_num_rows($qrysl);
				if($numsl!=0)
				{
					...
					$id=$ftq['id'];
					$cSession = curl_init();
					$sqlqry="select id from alarm where device_name like '%IO%' and field IN ($ffields) and altype='fire' and ack_datetime='null'";
				...
				if(!isset($_REQUEST['request']))
				{
					...
					if($num==0)
					{
						...
						{
							$send = curl_init();
							echo $ack = str_replace(" ","%20","http://$ipaddr/$plant/ScadaApi/acknowledge?alarmid=All&ackdate=$ackDate&type=alarm&altype=fire&request=server");
							curl_setopt($send,CURLOPT_URL,$ack);
							curl_setopt($send,CURLOPT_RETURNTRANSFER,true);
							curl_setopt($send,CURLOPT_HEADER, false);
							curl_setopt($send,CURLOPT_TIMEOUT,30);
							$result=curl_exec($send);
							if($result){echo "<br>Ack requested to $ipaddr<br>";}else{echo "<br>Ack not requested to $ipaddr<br>";}
							curl_error($send);
							curl_close($send);
						}
						}
		...
		private function report_kv()
	{
		...
		if(isset($_REQUEST['display']))
		{
		 ...
		 $sclqry="select a.DisplayName,a.Device,'EAE',FROM_UNIXTIME(a.lastts,'%Y-%m-%d %H:%i') AS ts,a.value FROM accord_em_report as a 
		where ReportDate='".$startTime."'  and field='EAE' and timeinhour >=6 and timeinhour <=19 $where order by Device,DisplayName,timeinhour";
		$sql = mysql_query($sclqry, $this->db);
		if(mysql_num_rows($sql) > 0)
		{
			while($rlt = mysql_fetch_array($sql,MYSQL_ASSOC)){
		 ...
		 $sclqry1="select block,display,field,FROM_UNIXTIME(hourts,'%Y-%m-%d %H:%i') AS ts,value FROM hourly_compressedvalue  
		 where ts='".$startTime."' and display='Irradiation' and hour >=6 and hour <=19 order by block,display,hour";
		$sql1 = mysql_query($sclqry1, $this->db);
		if(mysql_num_rows($sql1) > 0)
		{
			while($rlt1 = mysql_fetch_array($sql1,MYSQL_ASSOC)){
  ...
  if(mysql_num_rows($sql3) > 0)
  {
   ...
   else
   {
	   $startTime = mktime(0, 0, 0, $slmnth, $sdd, $crtyr);
		...
		$sclqry="select insertedts,device_name,field,value as value from ".$tbl." where ts >= $startTime and ts < $endTime AND field IN ('PAC','SOLAR_RADIATION') $where $groupby order by device_name,ts";
   }
   		$result = array();
		$sql = mysql_query($sclqry, $this->db);
		if(mysql_num_rows($sql) > 0)
