...
if($tpe=="month")
   ...
   else
   {
	...
	private function report_mainpage()
		...
		$sclpac="select block,device,field,FROM_UNIXTIME(ts) as ts,value FROM `day_variable` WHERE ts='".$startTime."' $where1 and `field`IN('PAC_MAX','PR_DAY','SOLAR_RADIATION_CUM','SOLAR_RADIATION_MAX','SOLAR_RADIATION_AVG','AMBIENT_TEMP_AVG','OPERATIONAL_TIME','PLANT_AUXILIARY') order by FIELD(field, 'PAC_MAX','PR_DAY','RADIATION_CUM','SOLAR_RADIATION_MAX','SOLAR_RADIATION_AVG','AMBIENT_TEMP_AVG','OPERATIONAL_TIME','PLANT_AUXILIARY')";
